...
const Header = ({updateUsersData}) => {

  const [showDropdown, setShowDropdown] = useState(false);

  const [burger_class, setBurgerClass] = useState(
    `${classes["burger-bar"]} ${classes["unclicked"]}`
  );
            ...
            <div className={classes.profile}>
              <div className={classes["login-dropdown-block"]}>
                <div className={classes["login-dropdown"]}>
                  {getAuthId !== "" ? (
                    <img src={updateUsersData && updateUsersData?.profile.profile_picture !== ""
                          ? updateUsersData && updateUsersData?.profile.profile_picture
                          : avatarcircle}
                      className={classes["avatar-cirlce"]}
                      alt="avatar-circle"
                    />
                  ) : (
                    <img
                  ...
                  {getAuthId !== "" ? (
                    <p className={classes["login-name"]}>
                      {updateUsersData && updateUsersData?.profile.firstname !== ""
                        ? updateUsersData && updateUsersData?.profile.firstname
                        : "Login"}
                    </p>
                  ) : (
                ...
                <Card
                  ...
                  style={{ display: showDropdown ? "none" : undefined }}
                >
                  <h6 className={classes["dropdown-title"]}>
                    FOOD IS MEDICINE
                  </h6>
                  {logout && (
              ...
              <div className={classes["shop-cart"]}>
                <div className={classes["shop-cart-image-block"]}>
                  <img
                    src={shopcart}
                    className={classes["shop-cart-image"]}
                    alt="shop-cart"
                  />
